<% control Top.Cart %>
	...
	<% if Quantity %>
		<tr class="item" id="ShopItem{$ID}" key="$ID">
			...
			<td class="title"><a href="$OriginalItem.Link">$Title <% if Option %><% control Option %>($Title <% if PriceDifference==0 %><% else %>$PriceDifferenceText $Order.Currency<% end_if %> )<% end_control %><% end_if %></a></td>
